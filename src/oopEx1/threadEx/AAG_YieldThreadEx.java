...
/*
 * run() 내부의 로직이 의미없는(연산을 하지 않는) 반복 작업을 수행시엔 yield() 를
 * 호출하여 자신을 대기 상태로 전환하도록 하고, 타 쓰레드에게 CPU 점유를 양보하도록
 * 제어하는 메서드입니다.
 * 
...
class WorkerThread extends Thread{
	...
	public void run() {
		while(true) {
			if (work) {//상태값에 따른 루프 분기작업.. 즉, work가 true일 때만 연산을 한다.
				System.out.println(getName() + "쓰레드가 연산을 수행함..");
			}else {
				//이 조건에서는 아무것도 할 게 없으니, 다른 쓰레드에게 CPU 점유를 양보함
				Thread.yield();
			}
		}
	}
...
public class AAG_YieldThreadEx {

	public static void main(String[] args) {
		/*
		 * WorkerThread 2개를 생성해서 하나에게는 루프의 flag 값을 false 로 주고
		 * 이후에 경쟁쓰레드가 CPU를 점유하는 상황을 보세요. 이후 다시 flag 를 true로 주면
		...
		wThread1.work = false; // <--- 쓰레드의 while() 에서 else절로 빠지고
							   // yield() 를 호출하여 실행 대기로 양보하게끔 한다.
