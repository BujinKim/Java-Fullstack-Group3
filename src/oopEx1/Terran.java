...
/*
 * 자바 추상화(Abstraction) 
 * 자바 추상화란 특정 메서드가 자신을 위한것이 아닌, 오로지
 * 상속만을 위한 것으로 생각될 때, 해당 메서드를 추상화 할 수 있습니다.
 * 이렇게 추상화된 메서드를 추상메서드라 하고, 이 메서드는 abstract라는 키워드를 통해 선언을 하게 됩니다.
 * 
 * 이렇게 클래스내에 추상메서드가 하나 이상 포함되면 해당 클래스는 명시적으로 추상 클래스로
 * 선언이 되어야 합니다. (안그러면 컴파일 에러)
 * 
 * 이렇게 추상 메서드로 선언이 된 클래스는 
 * 그 이후부터 절대 자신의 인스턴스를 생성할 수 없습니다. 
 * 
 ...
 * 
 * 따라서 자신의 인스턴스를 못 만들뿐, 나머지는 모두 같습니다.
 * 
 * 또한, 추상 클래스를 상속받은 자식클래스는 부모가 선언한 추상 메서드를
 * 명시적으로(무조건) 오버라이드해야 합니다.
 * 
...
public abstract class Terran extends Object {
	 ...
	 * 내 어디에도 사용될 수 있습니다.
	 * 
	 * 이름에서도 알 수 있듯이, final 키워드 뭔가를 고정시키는 역항릉 합니다.
	 * 클래스에 부틍면 상속금지, 메서드에 붙으면 오버라이드금지, 필드에 붙으면 고정값(상수화)
	 * 메서드 파리미터에 붙으면 메서드 내부에서 상수화 의미로 쓰입니다.
	 * 
...
//	final int GHOST_MAX_HP = 40;
	...
	//내 자식 객체가 생성시에 타입을 초기화 하기 위해 생성자 정의합니다.
	
	public Terran(String theUnit) {
		this.theUnit = theUnit;
	}
	
	//모든 유닛은 기본적으로 사운드로 리액션을 할 수 있도록 "상속"목적으로 sound() 메서드 정의함.
	//이 메서드는 Terran 입장에서 보면 내용부를 정의 할 수 없다..
	//때문에 상속을 목적으로만 존재하므로, 추상화의 대상이 된다.
	//추상메서드는 abstract 키워드와 함께 선언한 할 수 있습니다.
	//즉 내용부(메서드 바디)는 가질 수 없습니다.
