 ...
 * 던지는 형태를 말합니다.
 * 
 * 이때 사용되는 형태는 대표적으로 알려진 or 실행예외 2개 정도를 만들어 쓸 수 있습니다.
 * 
 * 알려진 예외는 Exception의 자식으로 클래스를 정의하여서 객체를 만들어 사용하는 형태이고,
 * 실행 예외는 Runtime을 상속받아 필요시 던지는 형태입니다.
 */

//아래의 클래스는 Exception 을 상속 받은 알려진 예외(Checked)로 정의합니다.
...
//계좌 클래스를 정의하고, 인출(withdraw()) 시에 잔액이 부족할 경우 예외를 던지도록 합니다.
...
class Accout{
	...
	public int withdraw(int money) throws AccException {
		//조건 절에서 인출 금액이 잔액보다 클 경우 에외를 던집니다.
		if(balance < money) {
			throw new AccException("잔액이 부족합니다 " + (money - balance) + "이 모자랍니다!!");
		}
