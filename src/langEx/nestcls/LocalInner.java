 ...
 * 							   정의하는데 있습니다.
 * 							   주의해야 할 점은 이 로컬 클래스는 메서드에서만 존재하기 때문에 메서드가 실행이 되지않을 경우엔
 * 							   존재 자체가 없습니다. 반대로 메서드가 실행이 끝난 경우에도 존재가 사라집니다.
 * 							   즉 외부에는 절대 노출이 안된다는 뜻입니다.
 * 
 * 이 로컬 클래스는 JDK 버전 없이 되기전에는 상수(final)만 가질 수 있었는데, 아마 15?이상 부터는
...
public class LocalInner {
	...
	void yourMethod(int gg) {
		/*
		 * 로컬 inner 클래스에서 접근 가능한 지역 변수는 반드시 final 이어야합니다.
		 * 단 jdk 10x 버전 이후부터는 이 제한이 풀렸습니다.
		 */
		int localVar = 100;
	...
	public static void main(String[] args) {
		...
		//LocalInner의 InnerInstance 타입의 instance 를 생성해보세요.
