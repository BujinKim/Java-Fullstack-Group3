...
//		System.out.println(f.getAbsolutePath());
		...
		//press란 폴더를 생성해 봅시다.
		String fName = "press";
		//생성자 파람 1개짜리 이용, File 객체 생성
		File folder = new File(fName);
		File file = new File("myFile.txt");
		if(!folder.exists()) {
			System.out.println(folder.mkdir()); //true
		}else {
			//만약 존재한다면.. 경로는 어떻게 되니?
			...
			System.out.println(newFile.createNewFile() 
					== true ? " 파일이 생성됨" : "뭔가 오류가 있음."); //뭔가 오류가 있음이 뜨는 것은 파일이 이미 생성되어있기 때문
			...
			System.out.println("숨김(hidden) : " + newFile.isHidden());
			...
			if(files.length != 0) {
				for (File fi : files) {
					...
					System.out.println(fi.delete() == true ? "삭제했습니다" : "삭제 안됨");
				}//end of for
			}//end of inner if
