...
public class StringQuiz {
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			if (isEmail(email)) {
				id = email.substring(0, email.indexOf('@')); // email의 @전까지 받음
				server = email.substring(email.indexOf('@') + 1, email.length()); // email의 @부터 받음
				
				int upperChar = 0; //대문자수를 카운트 하는 변수
				int digit = 0;//숫자를 카운트 하는 변수
